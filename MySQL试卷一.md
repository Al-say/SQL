...
1. 关系规则的完整性规则包括________、________和________

2. 数据库的三级模式结构中，外模式/模式映像可以保证数据和应用程序间的________，模式/内模式映像可以保证数据和应用程序间的________。

3. 并发控制的主要方法是封锁，基本的封锁类型有________和________

4. 关系代数运算中，专门的关系运算有________、________、________

5. 需在存储过程中使用某一参数向存储过程传入数值，同时该参数还存储了需要返回的计算结果，则该参数在存储过程参数列表中的修饰符为________。
